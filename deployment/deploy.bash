#!/bin/bash
echo "FOR FRESH INSTALLS ONLY.  MODIFIED BY NC0D3R.  If you're not, please exit in the next 15 seconds."
sleep 15
...
cd ~
git clone https://github.com/nc0d3r/xmr-pool.git  # Change this depending on how the deployment goes.
cd /usr/src/gtest
sudo cmake .
sudo make
...
cd ~
git clone https://github.com/nc0d3r/poolui.git
cd poolui
npm install
...
sudo cp ~/xmr-pool/deployment/caddyfile /etc/caddy/Caddyfile
...
bash ~/xmr-pool/deployment/install_lmdb_tools.sh
cd ~/xmr-pool/sql_sync/
